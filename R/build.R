...
build_problem_sets <- function(problem_sets) {
...
build_lectures <- function(lectures) {
  ...
  for (lecture in lectures) {
    ...
    post_file <- file.path(root_dir, "_lectures", paste0(lecture, ".md"))
...
build_practice_activities <- function() {
...
build <- function(to_build) {

  
  collections <- yaml.load_file(file.path(root_dir, "_config.yml"))$collections
  
  #### Building Problem Sets Collection ####
  problem_sets <- collections$problem_sets$order
  
  if ("problem_sets" %in% to_build) {
    build_problem_sets(problem_sets = collections$problem_sets$order)
  }
  
  if ("lectures" %in% to_build) {
    build_lectures(lectures = collections$lectures$order)
  }
  
  if ("practice_activities" %in% to_build) {
    build_practice_activities()
  }  
...
args <- tolower(trimws(unlist(strsplit(args, ","))))

all <- c("problem_sets", "lectures", "practice_activities")
if (length(args)==0 || "all" %in% args) {
  to_build <- all
} else {
  to_build <- match.arg(args, all, several.ok = TRUE)
}

cat(sprintf("Building: %s", paste(to_build, collapse = ", ")), sep = "\n")
build(to_build)