---
title: "Finding Confidence Intervals"
author: "Will Hopper"
output:
...
knitr::opts_chunk$set(echo = FALSE, cache=TRUE, fig.align = 'center', dev="svg")
...
```{r, include=FALSE}
x <- ggplot(data.frame(x=seq(-5.1, 5.1, .1)), aes(x=x)) +
  geom_hline(yintercept = 0) +
  stat_function(fun = dnorm, n = 201, color="red",size=1) +  
  scale_x_continuous(breaks=seq(-4,4,1), expand = c(0,0)) +
  theme(axis.title = element_blank())

x2 <- x + 
  scale_x_continuous(breaks=seq(3.5,4,.1)) +
  coord_cartesian(xlim=c(3.5, 4), ylim=c(0,.01)) +
  theme(panel.border = element_rect(fill=NA,color="blue", size=2, 
                                    linetype="solid"))

x3 <- x + 
  scale_x_continuous(breaks=seq(4,4.5,.1)) +
  coord_cartesian(xlim=c(4,4.5), ylim=c(0,.001)) +
  theme(panel.border = element_rect(fill=NA,color="blue", size=2, 
                                    linetype="solid"))

x4 <- x + 
  scale_x_continuous(breaks=seq(4.5,5,.1)) +
  coord_cartesian(xlim=c(4.5,5), ylim=c(0,.0001)) +
  theme(panel.border = element_rect(fill=NA,color="blue", size=2, 
                                    linetype="solid"))
```
...
grid.arrange(x + geom_point(shape=21, x=3.75, y=0, size=8, color="blue"), x2, nrow=1)
```
...
```{r, fig.width=10, fig.height=5}
grid.arrange(x + geom_point(shape=21, x=4.25, y=0, size=8, color="blue"), x3, nrow=1)
```
...
```{r, fig.width=10, fig.height=5}
grid.arrange(x + geom_point(shape=21, x=4.75, y=0, size=8, color="blue"), x4, nrow=1)
```
...
```{r, fig.height=6}
heights <- dnorm(seq(-1.96, 1.96, .01))
shaded_x <- x + geom_polygon(data=data.frame(x=c(-1.96, seq(-1.96, 1.96, .01), 1.96),
                                             y=c(0, heights - .0015, 0)
                                 ),
                             mapping=aes(x=x,y=y),
                             fill="royalblue",
                             stat="identity"
                             )  +
  geom_polygon(data=data.frame(x=c(-3.5, seq(-3.5, -1.96, .01), -1.96),
                               y = c(0, dnorm(seq(-3.5, -1.96, .01)) - .0015, 0)
                              ),
               mapping = aes(x = x, y = y),
               fill = "grey"
               ) +
  geom_polygon(data=data.frame(x=c(1.96, seq(1.96, 3.5, .01), 3.5),
                               y = c(0, dnorm(seq(1.96, 3.5, .01)) - .0015, 0)
                              ),
               mapping = aes(x = x, y = y),
               fill = "grey"
               )
...
shaded_x +
  geom_segment(x=-1.96, xend=1.96, y=.025, yend=.025, size=2, arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')) +
  annotate(geom = "label", label="95% of the distribution", x=0, y=.05, size = 5)
...
shaded_x + 
  geom_segment(x=-1.96, xend=1.96, y=.025, yend=.025, size=2, arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')) +
  annotate(geom = "label", label="95% of the distribution", x=0, y=.05, size = 5) +
  geom_segment(x=-2.5, xend=-1.96, y=.1, yend=.04,
               arrow = arrow(length = unit(0.02, "npc"))
               ) +
  annotate(geom = "label", label="lower = ??", x=-2.5, y=.1, size = 5) +  
  geom_segment(x=2.5, xend=1.96, y=.1, yend=.04,
               arrow = arrow(length = unit(0.02, "npc"))
               ) +
  annotate(geom = "label", label="upper = ??", x=2.5, y=.1, size = 5)
```
]
...
```{r, fig.height=6}
shaded_x +
    annotate(geom = "label", label="2.5% of the\ndistribution", x=3.5, y=.05, size = 5) +
    geom_segment(x=-3.25, xend=-2.75, y=.03, yend=.01,
                 arrow = arrow(length = unit(0.02, "npc"))
                 ) +
    annotate(geom = "label", label="2.5% of the\ndistribution", x=-3.5, y=.05, size = 5) +
    geom_segment(x=3.25, xend=2.75, y=.03, yend=.01,
...
```{r, fig.height=6}
shaded_x + 
  geom_segment(x=-5, xend=-1.96, y=.025, yend=.025, size=2, arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'last')) +
  annotate(geom = "label", label="2.5% of the\ndistribution", x=-3.5, y=.05, size = 5) +
  geom_segment(x=-2.5, xend=-1.96, y=.1, yend=.04,
               arrow = arrow(length = unit(0.02, "npc"))
               ) +
...
shaded_x + 
  geom_segment(x=-5, xend=1.96, y=.025, yend=.025, size=2, arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'last')) +
  annotate(geom = "label", label="97.5% of the\ndistribution", x=-3.5, y=.05, size = 5) +
  geom_segment(x=2.5, xend=1.96, y=.1, yend=.04,
               arrow = arrow(length = unit(0.02, "npc"))
               ) +
  annotate(geom = "label", label="upper", x=2.5, y=.1, size = 5)
...
bootstrap_ci <- bootstrap_dist %>%
  summarise(lower = quantile(avg_weights, .025),
            upper = quantile(avg_weights, .975)
            )
attr(bootstrap_dist, "response") <- "avg_weight"
attr(bootstrap_dist, "response_type") <- "numeric"
attr(bootstrap_dist, "distr_param") <- c(df=499)
attr(bootstrap_dist, "theory_type") <- "One sample t"
attr(bootstrap_dist, "generate") <- TRUE
attr(bootstrap_dist, "type") <- "bootstrap"
attr(bootstrap_dist, "stat") <- "mean"
infer::visualize(rename(bootstrap_dist, stat=avg_weights)) +
  infer::shade_ci(endpoints = bootstrap_ci)
```
]
...
.pull-right[
```{r, message=FALSE, fig.height=5.5, fig.width=5.5}
std_normal <- x +
  geom_polygon(data=data.frame(x=c(-1.96, seq(-1.96, 1.96, .01), 1.96),
                                             y=c(0, heights - .0015, 0)
                                 ),
               mapping=aes(x=x,y=y),
               fill="royalblue",
  ...
  scale_x_continuous("x = standard deviations", breaks=seq(-4,4,1), expand = c(0,0)) +
  scale_y_continuous(limits=c(-.03, .4))+
  geom_segment(x=-1.96, xend=1.96, y=.025, yend=.025, size=2,
               arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
               ) +
  annotate(geom = "label", label="95% of the\ndistribution", x=0, y=.05, size = 5) +
  geom_segment(x=-1.96, xend=-1.96, y=-.03, yend=0.01, arrow=arrow(angle=30, length = unit(0.03, "npc")), size=1) +
  annotate(geom="label", label="-1.96", x=-1.96, y=-.03, size=5) +
  geom_segment(x=1.96, xend=1.96, y=-.03, yend=0.01, arrow=arrow(angle=30, length = unit(0.03, "npc")), size=1) +
  annotate(geom="label", label="1.96", x=1.96, y=-.03, size=5) +
...
.pull-right[
```{r, message=FALSE, fig.height=6.5, fig.width=5.5}
CLT_estimate <-
  ggplot(data.frame(x=seq(-3, 7.5, .1)), aes(x=x)) +
  stat_function(fun = dnorm, args = list(mean=mu_hat_x_bar, sd=sqrt(sigma_sq_hat_x_bar)),
                color="forestgreen", size=2
                ) +
  scale_x_continuous("Average Birth Weight", breaks=seq(-3,7,1)) +
...
xnew <- ggplot(data.frame(x=seq(-3.5, 7.5, .1)), aes(x=x)) +
  geom_hline(yintercept = 0) +
  stat_function(fun = dnorm, n = 201, color="red",size=1) +
  scale_x_continuous("x = standard deviations",breaks=seq(-3,7,1)) +
...
grid.arrange(xnew, CLT_estimate, nrow=2)
```
]
...
```{r, message=FALSE, fig.height=5.5, fig.width=5.5}
grid.arrange(xnew +
               geom_segment(x=-1.96, xend=1.96, y=.025, yend=.025, size=1, color="grey",
                            arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
                            ) +
               geom_segment(x=-1.96 + 7.1, xend=1.96 + 7.1, y=.025, yend=.025, size=1,
                            arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
                            ) +
               geom_segment(x=0, xend=7.2, y=.1, yend=.1, size=1, color="grey",
                            arrow = arrow(angle=30, length = unit(0.03, "npc"))
                            ) +
               annotate(geom='label', label='0 + 7.1', x=3.5, y=.15, size=5),
             CLT_estimate, nrow=2)
```
]
...
grid.arrange(xnew +
               ...
               geom_segment(x=-1.96 + 7.1, xend=1.96 + 7.1, y=.025, yend=.025, size=1, color="grey",
                            arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
                            ) +
               geom_segment(x=sqrt(sigma_sq_hat_x_bar)*-1.96 + 7.1,
                            xend=sqrt(sigma_sq_hat_x_bar)*1.96 + 7.1,
                            y=.025, yend=.025, size=1,
                            arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
                            ) +
               annotate(geom='label', label='1 %*% sqrt(.0022)', x=6.75, y=.15, size=5, parse=TRUE),
             CLT_estimate,
             nrow=2)
```
]
...
.pull-right[
```{r message=FALSE}
lower <- qnorm(.025, mu_hat_x_bar, sqrt(sigma_sq_hat_x_bar))
upper <- qnorm(.975, mu_hat_x_bar, sqrt(sigma_sq_hat_x_bar))

CLT_estimate +
  scale_x_continuous("Average Birth Weight", limits=c(6.9, 7.3), breaks=seq(7, 7.3, .1)) +
  scale_y_continuous(limits=c(-1.5, 8.4)) +
  geom_segment(x=upper, xend=lower,
               y=.025, yend=.025, size=1,
               arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
               ) +
  geom_segment(x=lower, xend=lower, y=-1.5, yend=-.45,
               arrow = arrow(angle=30, length = unit(0.03, "npc"))
               ) +
  geom_segment(x=upper, xend=upper, y=-1.5, yend=-.45,
               arrow = arrow(angle=30, length = unit(0.03, "npc"))
               ) +
  annotate("label", label=paste("7.1 + 1.96 %*% sqrt(.0022) ==", round(upper,3)),
           x=upper, y=-1.5, parse=TRUE, size=5) +
  annotate("label", label=paste("7.1 - 1.96 %*% sqrt(.0022) ==", round(lower,3)),
           x=lower, y=-1.5, parse=TRUE, size=5)
```
]
