...
```{r, fig.height=5.5, message=FALSE, dev="svg"}
mu <- mean(ncbirths_population$weight)
sigma2 <- var(ncbirths_population$weight)

true_sampling <- ggplot(data=data.frame(x=c(7,7.4)), aes(x=x)) +
  stat_function(fun=dnorm, args=list(mu, sqrt(sigma2/1000)), color="red", n=101, size=1.5) +
  scale_x_continuous("Average Birth Weight (N=1,000)") +
  geom_hline(yintercept = -0.02, color="lightgrey", size=1) +
...
annotations <- function(x) {
  y <- x +
  geom_segment(x=mu, xend=mu, y=0, yend=dnorm(mu, mu, sqrt(sigma2/1000)),
               size=1) +
  geom_segment(x=mu, xend=7.25, y=dnorm(7.25, mu, sqrt(sigma2/1000)), yend=dnorm(7.25, mu, sqrt(sigma2/1000)),
               size=1) +
  annotate('label', label="{mu[bar(x)]== mu}==7.205", x=mu, y=9.5, parse=TRUE, size=5) +
  annotate('label', label="{sigma[bar(x)]^2== frac(sigma^2, N)}==0.0019", x=7.31, y=dnorm(7.25, mu, sqrt(sigma2/1000)),
           parse=TRUE, size=5)
  return(y)
}

annotations(true_sampling)
```
...
```{r, fig.height=5.5, message=FALSE, dev="svg"}
upper <- qnorm(.975, mu, sqrt(sigma2/1000))
lower <-  qnorm(.025, mu, sqrt(sigma2/1000))

shaded_true <- true_sampling +
 geom_polygon(data=data.frame(x=c(lower, seq(lower, upper+.001, .001),                upper),
                              y=c(0,     dnorm(seq(lower, upper+.001, .001), mu, sqrt(sigma2/1000)) - .0015, 0)
                              ),
                             mapping=aes(x=x,y=y),
                             fill="royalblue",
                             stat="identity"
                             )  +
  geom_polygon(data=data.frame(x = c(7.37,       seq(7.03, lower, .001),                                lower),
                               y = c(0, dnorm(seq(7.03, lower, .001), mu, sqrt(sigma2/1000)) - .0015, 0)
                              ),
               mapping = aes(x = x, y = y),
               fill = "grey"
               ) +
  geom_polygon(data=data.frame(x=c(upper, seq(upper, 7.37, .001), 7.37),
                               y = c(0, dnorm(seq(upper, 7.37, .001), mu, sqrt(sigma2/1000)) - .0015, 0)
                              ),
               mapping = aes(x = x, y = y),
               fill = "grey"
               )
 
annotations(shaded_true)
```
...
```{r, fig.height=5.5, message=FALSE, dev="svg"}
annotations(shaded_true) +
  annotate('label', label="95% Confidence Interval", x=mu, y=.5, size=5) +
  geom_segment(x=upper, xend=lower,
               y=0, yend=0, size=1,
               arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
               ) +
  geom_segment(x=lower, xend=lower, y=-1.25, yend=-.45,
               arrow = arrow(angle=30, length = unit(0.03, "npc"))
               ) +
  geom_segment(x=upper, xend=upper, y=-1.25, yend=-.45,
               arrow = arrow(angle=30, length = unit(0.03, "npc"))
               ) +
  annotate("label", label=paste("7.205 + 1.96 %*% sqrt(.0019) ==", round(upper,2)),
           x=upper+.025, y=-1.5, parse=TRUE, size=5) +
  annotate("label", label=paste("7.205 - 1.96 %*% sqrt(.0019) ==", round(lower,2)),
           x=lower-.025, y=-1.5, parse=TRUE, size=5) +
  scale_y_continuous(limits=c(-1.5,9.5), breaks = seq(0,8,2))
```
...
95% of all sample means will be between `r round(upper,2)` and `r round(lower,2)`.
```{r, fig.height=5.5, message=FALSE, dev="svg"}
simplified_CI <- shaded_true +
  geom_segment(x=upper, xend=lower,
               y=0, yend=0, size=1,
               arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both')
               ) +
  ...
  annotate("label", label=round(upper,2), x=upper, y=-1.5,size=5) +
  annotate("label", label=round(lower,2), x=lower, y=-1.5, size=5) +
  scale_y_continuous(limits=c(-1.5,9.5), breaks = seq(0,8,2))  +
  geom_segment(x=mu, xend=mu, y=0, yend=dnorm(mu, mu, sqrt(sigma2/1000)),
               size=1) +
  annotate('label', label="{mu[bar(x)]== mu}==7.205", x=mu, y=9.5, parse=TRUE, size=5)
...
```{r, fig.height=5.5, message=FALSE, dev="svg"}
set.seed(12)
x <- sample_n(select(ncbirths_population, weight), 1000)
x_bar <- mean(x$weight)
s2 <- var(x$weight)
mu_hat_x_bar <- x_bar
sigma2_hat_x_bar <- s2/1000
upper_hat <- qnorm(.975, mu_hat_x_bar, sqrt(sigma2_hat_x_bar))
lower_hat <- qnorm(.025, mu_hat_x_bar, sqrt(sigma2_hat_x_bar))

shaded_true +
  geom_vline(xintercept = x_bar, size=1, color="forestgreen") +
  geom_segment(x=mu, xend=mu, y=0, yend=dnorm(mu, mu, sqrt(sigma2/1000)),
               size=1) +
  annotate('label', label="{mu[bar(x)]== mu}==7.205", x=mu, y=9.5, parse=TRUE, size=5)
```
...
Based on our sample, we *estimate* the CI to range from `r round(upper_hat,2)` to `r round(lower_hat,2)`
```{r, fig.height=5.5, message=FALSE, dev="svg"}
...
shaded_true +
  ...
  geom_segment(x=mu, xend=mu, y=0, yend=dnorm(mu, mu, sqrt(sigma2/1000)),
               size=1) +
  annotate('label', label="{mu[bar(x)]== mu}==7.205", x=mu, y=9.5, parse=TRUE, size=5) +
  geom_segment(x=upper_hat, xend=lower_hat,
               y=0, yend=0, size=1,
               arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both'),
               color="forestgreen")
```
...
```{r, message=FALSE, fig.retina=2, fig.height=5.5, out.height=530, gganimate = list(nframes = 50, fps = 7)}
hypotheticals <- data.frame(x_bar = seq(x_bar, 7.1, length.out = 50)) %>%
  mutate(lower_hat = qnorm(.025, x_bar, sqrt(sigma2_hat_x_bar)),
         upper_hat = qnorm(.975, x_bar, sqrt(sigma2_hat_x_bar)),
         frame = 1:nrow(.),
         mu=mu,
         alpha = ifelse(upper_hat < mu, .25, 1)
         )

animated_ci <- shaded_true +
  geom_vline(data=hypotheticals, mapping=aes(xintercept = x_bar), size=1, color="forestgreen") +
  geom_segment(data=hypotheticals, mapping=aes(alpha=alpha),
               x=mu, xend=mu, y=0, yend=dnorm(mu, mu, sqrt(sigma2/1000)),
               size=1, show.legend = FALSE
               ) +
  annotate('label', label="{mu[bar(x)]== mu}==7.205", x=mu, y=9.5, parse=TRUE, size=5) +
  geom_segment(data=hypotheticals,
               mapping=aes(x=lower_hat, xend=upper_hat),
               y=0, yend=0, size=1,
               arrow = arrow(angle=90, length = unit(0.03, "npc"), ends = 'both'),
               color="forestgreen"
               )  +
...
#x <- animate(animated_ci, fps=7, nframes = nrow(hypotheticals))
#knit_print.gganim(x, options=list(fig.height=5.5))
```
...
.pull-right[
```{r, fig.retina=2, out.height=540, out.width=540, fig.width=7.5, fig.height=7.5, gganimate = list(nframes = 50, fps = 7)}
animated_ci
```
]
...
```{r, fig.width=12, dev='svg', fig.height=5}
ggplot(sample_means) +
  geom_segment(aes(
    y = replicate, yend = replicate, x = lower, xend = `upper`, 
    color = factor(mu_is_in, levels = c("TRUE", "FALSE"))
  )) +
  scale_color_manual("Captured Mu", values=c("lightgrey","red"), labels=c("Yes", "No")) +
  geom_point(aes(x = weight_avg, y = replicate, color = mu_is_in)) +
  labs(x = expression("Average Birth Weight"), 
...
Hey, not bad, by adding up all the TRUEs and FALSES, which act like 1's and 0's, we find that 96 of our 100 confidence intervals captured the population mean. Let's visualize our simulation by putting all 100 confidence intervals on the same plot, along with the population mean.

Each vertical line is a confidence interval, and the horizontal black line is the population mean. If a confidence interval is shown with a red line, that means it didn't capture the population mean - which is easy to see in this plot, the black horizontal line for the population mean doesn't intersect with the confidence interval for those 4 samples.
