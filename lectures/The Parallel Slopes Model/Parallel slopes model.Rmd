...
---
title: "An Additive Multiple Regression Model"
output:
...
smoker_summary <- group_by(ncbirths, smoker) %>%
  summarise(weight = mean(weight))
```

```{r, echo=FALSE, dev="svg", fig.width=10, fig.height=6}
weeks_plot <- ggplot(aes(x=weeks,y=weight), data = ncbirths) +
  geom_point() +
  geom_smooth(method="lm", se=FALSE) +
  ggtitle("weight ~ weeks") +
  theme(plot.title = element_text(size=16), plot.subtitle = element_blank())

smoker_plot <- ggplot(aes(x=smoker,y=weight, color=smoker), data = ncbirths) +
  geom_point(position = position_jitter(width = .25)) +
  geom_segment(aes(x = as.numeric(smoker)-.5, xend = as.numeric(smoker)+.5, y=weight, yend=weight),
               data = smoker_summary, color="blue") +
  scale_x_discrete(expand = c(0,.5)) +
  scale_y_discrete("") +
  guides(color = FALSE) +
  ggtitle("weight ~ smoker") +
  theme(plot.title = element_text(size=16), plot.subtitle = element_blank())

grid.arrange(weeks_plot, smoker_plot, ncol=2)
```
...
```{r, echo=FALSE, dev="svg", fig.width=10, fig.height=6}
both_plot <- ggplot(aes(x=weeks,y=weight, color=smoker), data = ncbirths) +
  geom_point(position = position_jitter(.25), alpha=.5) +
  geom_parallel_slopes(se=FALSE) +
  ggtitle("weight ~ smoker + weeks") +
  theme(plot.title = element_text(size=16), plot.subtitle = element_blank())
...
.right-column[
```{r, echo=FALSE, dev='svg', fig.height=6, fig.align='right'}
fullrange_plot <-
  ggplot(aes(x=weeks,y=weight, color=smoker), data = ncbirths) +
    geom_point(position = position_jitter(.25), alpha=.5) +
    geom_abline(data=data.frame(smoker="non-smoker"),
                mapping = aes(color=smoker,
                              slope = smoker_weeks_parallel_model$coefficients['weeks'],
                              intercept = smoker_weeks_parallel_model$coefficients['(Intercept)']
                              )
                ) +
    geom_abline(data=data.frame(smoker="smoker"),
                mapping = aes(color=smoker,
                              slope = smoker_weeks_parallel_model$coefficients['weeks'],
                              intercept = smoker_weeks_parallel_model$coefficients['(Intercept)'] +
                              smoker_weeks_parallel_model$coefficients['smokersmoker']
                              )
                ) +
...
.right-column[
```{r, echo=FALSE, dev='svg', fig.height=6, fig.align='right'}
fullrange_plot + coord_cartesian(xlim=c(32,38),ylim=c(5,7))
```
]
