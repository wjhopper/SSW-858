---
title: "Towards Hypothesis Testing of Regression Coefficients"
author: "Will Hopper"
output:
...
default_theme <-
  theme_set(theme_bw(36) +
            theme(plot.margin = margin(12,1,1,1),
                  legend.box.margin = margin(0,0,0,-20),
                  plot.title = element_text(size=32, margin = margin(-10)),
...
ncbirths_pop <- read.csv("../../data/ncbirths_population.csv", stringsAsFactors = FALSE) %>%
  select(weight, weeks) %>%
  dplyr::filter(!is.na(weight), !is.na(weeks), weeks < 60)

set.seed(100)
ncbirths_pop_sample <- sample_frac(ncbirths_pop, .1)
...
```{r include=FALSE}
lm(weight ~ weeks, ncbirths)
```

```{r fig.height=11, fig.width=11, dpi=96, out.width=528, out.height=528}
classic <- ggplot(ncbirths, aes(x=weeks, y=weight)) +
  geom_point(color="blue", size=3) +
  geom_smooth(method="lm", formula = y~x, color="red", se=FALSE, size=1.5) +
  scale_x_continuous(limits = range(ncbirths_pop$weeks),
                     breaks = seq(15,50,5)) +
  scale_y_continuous(breaks = seq(2,12,by=2),
                     limits = range(ncbirths_pop$weight)) +
  ggtitle("weight ~ weeks") +
...
```{r warning=FALSE, fig.height=11, fig.width=11, dpi=96, out.width=528, out.height=528, gganimate = list(nframes = 50, fps = 10)}

ggplot(ncbirths_pop_sample, aes(x=weeks, y=weight)) +
  geom_point(position = position_jitter(width=.25), shape=19, size =3) +
  geom_point(data = ncbirths, color="blue", size=3) +
  geom_smooth(data = ncbirths, method="lm", formula = y~x, color="red", se=FALSE, size=1.5) +
  scale_x_continuous(limits = range(ncbirths_pop$weeks)+c(-.5, .5),
                     breaks = seq(15,50,5)
                     ) +
  scale_y_continuous(limits = range(ncbirths_pop$weight)+c(-.05, .05)) +
  transition_layers(layer_order = c(2,3,1),
                    transition_length = c(0, 0, 2),
                    layer_length = c(0, 0, 1)
                    ) +
  ggtitle("weight ~ weeks") +
  enter_fade()
```
...
samples <- rep_sample_n(ncbirths_pop, size=1000, reps = 250) %>%
  ungroup()

models <- split(samples, samples$replicate) %>%
  lapply(function(df) { get_regression_table(lm(weight ~ weeks, data = df))}) %>%
  bind_rows(.id = "replicate") %>%
  select(replicate, term, estimate) %>%
  mutate(replicate = as.integer(replicate)) %>%
  tidyr::spread(key = term, value = estimate)

scatterplot_base <- ggplot(data = data.frame(weeks=NA_real_, weight=NA_real_),
                           mapping = aes(x=weeks, y=weight)
                           ) +
  scale_x_continuous(limits = range(ncbirths_pop$weeks),
                     breaks = seq(15,50,5)
                     ) +
  scale_y_continuous(limits = range(ncbirths_pop$weight)) +
  ggtitle("")

custom_theme <-
  theme_set(theme_bw(16) +
            theme(plot.margin = margin(12,5,5,5),
                  legend.box.margin = margin(0,0,0,-20),
...
img <- image_graph(1728, 576, res = 96, clip = FALSE)

for (i in 1:nrow(models)) {

  single_sample <- filter(samples, replicate == i)
  
  top <- ggplot(single_sample, aes(x=weeks, y=weight)) +
    geom_point(data = ncbirths_pop_sample) +
    geom_point(color = "blue") +
    scale_x_continuous(limits = range(ncbirths_pop$weeks)+c(-.5, .5),
                       breaks = seq(15,50,5)
                       ) +
    scale_y_continuous(limits = range(ncbirths_pop$weight)+c(-.05, .05)) +    
    ggtitle(paste0("Collect Sample (", i, " of ", nrow(models), ")"))
  
  bottom_left <- scatterplot_base %+% single_sample +
    geom_point(color="blue", size=1) +
    geom_smooth(method="lm", formula = y~x, color="red", se=FALSE, size=1) +
    ggtitle(paste0("Fit Model (", i, " of ", nrow(models), ")"))
  
  bottom_right <- ggplot() +
    geom_abline(aes(slope=weeks, intercept=intercept),
                data = filter(models, replicate < i),
                color = "blue", alpha=.25) +
    scale_x_continuous("weeks",
                       limits = range(ncbirths_pop$weeks)+c(-.5, .5),
                       breaks = seq(15,50,5)
                       ) +
    scale_y_continuous("weight",
                       limits = range(ncbirths_pop$weight)+c(-.05, .05)
                       ) +
    ggtitle(paste0("Record Slope (", i, " of ", nrow(models), ")"))
  
  bottom_right_plus_new_slope <- bottom_right +
    geom_abline(aes(slope=weeks, intercept=intercept),
                data = filter(models, replicate == i), 
                color="red", size=1
                ) +
  ...
  grid.arrange(grobs = list(top,
                            scatterplot_base + ggtitle(paste0("Fit Model (", i, " of ", nrow(models), ")")),
                            bottom_right
                            ),
...
```{r sampling_dist_hist, fig.width=18, fig.height=6}
...
bottom_right_hist <- ggplot(models, aes(x=weeks)) +
  geom_histogram(binwidth = .005, boundary=.26, fill = "blue", color="black") +
  ggtitle(paste0("Sampling Distribution (", i, " of ", nrow(models), ")"))
...
```{r}
two_vars <- select(ncbirths, weight, weeks)

make_one_perm <- function(x) {
  one_perm <- bind_rows(mutate(x, replicate=0),
                      mutate(x, replicate=1, weeks = sample(weeks))
                      ) %>%
  mutate(replicate = as.factor(replicate))
}

lots_of_perms <- replicate(64, make_one_perm(two_vars), simplify = FALSE) %>%
  bind_rows(.id="panel") %>%
  mutate(panel = as.numeric(panel))
```

```{r fig.height=11, fig.width=11, dpi=96, out.width=528, out.height=528, gganimate = list(nframes = 50, fps = 10)}
ggplot(filter(lots_of_perms, panel==1), aes(x=weeks, y=weight)) +
  geom_point(color="blue") +
  ...
  scale_y_continuous(breaks = seq(2,12,by=2),
                     limits = range(ncbirths_pop$weight)) +
  transition_states(replicate, wrap=FALSE)
```
...
```{r fig.height=12, fig.width=12, dpi=96, out.width=576, out.height=576, gganimate = list(nframes = 50, fps = 10)}
ggplot(filter(lots_of_perms, panel<=4), aes(x=weeks, y=weight)) +
  geom_point(color="blue") +
  geom_smooth(method="lm", formula = y~x, se = FALSE, color = "red") +
...
```{r fig.height=12, fig.width=12, dpi=96, out.width=576, out.height=576, gganimate = list(nframes = 50, fps = 10)}
ggplot(filter(lots_of_perms, panel<=16), aes(x=weeks, y=weight)) +
  geom_point(color="blue") +
  geom_smooth(method="lm", formula = y~x, se = FALSE, color = "red") +
  facet_wrap(~panel, nrow = 4, ncol = 4) +
  scale_x_continuous(limits = range(ncbirths_pop$weeks),
                     breaks = seq(15,50,5)) +
  scale_y_continuous(breaks = seq(2,12,by=2),
                     limits = range(ncbirths_pop$weight)) +
...
```{r fig.height=12, fig.width=12, dpi=96, out.width=576, out.height=576, gganimate = list(nframes = 50, fps = 10)}
ggplot(filter(lots_of_perms, panel<=64), aes(x=weeks, y=weight)) +
  geom_point(color="blue", size=.5) +
  geom_smooth(method="lm", formula = y~x, se = FALSE, color = "red", size=.5) +
  facet_wrap(~panel, nrow = 8, ncol = 8) +
  scale_x_continuous(limits = range(ncbirths_pop$weeks),
                     breaks = seq(15,50,5)) +
  scale_y_continuous(breaks = seq(2,12,by=2),
                     limits = range(ncbirths_pop$weight)) +
...
```{r fig.height=12, fig.width=12, dpi=96, out.width=576, out.height=576}
set.seed(50)
...
visualise(permutation_distribution) +
  scale_x_continuous("weight ~ weeks model slope",
                     breaks = seq(-.06, 0.06, .02)
                     )
```
...
```{r fig.height=12, fig.width=12, dpi=96, out.width=576, out.height=576}
visualise(permutation_distribution) +
  shade_p_value(.343, direction = "greater") +
  scale_x_continuous("weight ~ weeks model slope",
                     breaks = seq(-.06, 0.36, .06)
                     )
```
